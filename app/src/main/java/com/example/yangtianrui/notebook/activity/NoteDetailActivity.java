...
public class NoteDetailActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void initData() {
        ...
        mNote = new Note("", "",TextFormatUtil.formatDate(new Date()),TextFormatUtil.formatDate(new Date()));
        mNoteID = intent.getIntExtra(SENDED_NOTE_ID, -1);
        // 如果有ID参数,从数据库职工获取信息
        mNoteDAO = new NoteDAO(this);
        if (mNoteID != -1) {
            // 进行查询必须使用?匹配参数
            mCursor = mNoteDAO.queryNote("_id=?", new String[]{mNoteID + ""},null);
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.id_btn_modify) {
            ...
            values.put("modifyTime",modifydate);
            int rowID = -1;
            // 向数据库添加或者更新已有记录
            if (mNoteID == -1) {
